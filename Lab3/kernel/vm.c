...
// add a mapping to the user process kernel page table
void
...
// copy ptes from user pagetable to kernel pagetable
// map from oldsz to newsz
void kvmmapuser(pagetable_t kpagetable, pagetable_t upagetable, uint64 newsz, uint64 oldsz){
    ...
    for(va = oldsz; va < newsz; va += PGSIZE){
        // find level 0 pte of user pagetable
        upte = walk(upagetable, va, 0);
        if(upte == 0) {
            printf("kvmmapuser: 0x%x 0x%x", va, newsz);
            panic("kvmmap: no upte");
        }
        if((*upte & PTE_V) == 0) {
            printf("kvmmapuser: 0x%x 0x%x", va, newsz);
            panic("kvmmap: no valid pte");
        }
...
// create kernel pagetable for user process
pagetable_t
kvmcreate()
{
  pagetable_t pagetable = uvmcreate();
  // share some kernel pte
  // Lower 1GB is not shared, beacause it will be modified. 
  for(int  i = 1; i < 512; i++) {
...
// only need to free the first entry of the pagetable
// which is allocated by kvmcreate
void 
